...
> **Note on stale framing (May 2026).** This document is preserved as the original
> design intent, but several parts of its framing have been superseded as the
> codebase matured. Specifically:
>
> - The **"Dual-Persona Mentor / Junior SWE"** model has been retired as the
>   organising abstraction. The product is now structured as a 4-stage workflow
>   (Learn → Build → Verify → Approve), not as a conversation between two
>   personas. Only one real agent will exist in the runtime — the BuildAgent
>   (see `docs/009_build_agent_design.md`).
> - The **platform / package name** is no longer `learning-agent`. The runtime
>   package is `engine/`; each step is its own top-level package (`learn/`,
>   `build/`, `verify/`, `ui/`, `topic_chat/`, `curriculum/`). The user-facing
>   CLI brand is `coach`. See `docs/008_workspace_restructure.md`.
> - References to "the platform" generally mean the codebase as a whole; where
>   this document refers to a single `learning-agent/` package, the modern
>   equivalent is the entire repository, with responsibilities split across the
>   sibling packages.
>
> The pedagogy, gating model, ledger discipline, and phased delivery plan all
> still hold. The persona vocabulary remains useful for prompt flavoring
> (`engine/prompts/mentor.md`, `engine/prompts/junior.md`) but is no longer
> load-bearing in the code structure.
...
The ideal system operates across two logically separate workspaces (example below):
...
- receives only the currently unlocked week context,
- writes code and supporting artifacts for the active week,
- runs verification commands,
- reports results back to the Mentor,
- does not teach beyond the current week on its own.
...
- Values such as `active_dirs`, required artifacts, or required metrics may vary by week.
...
3. `validate_boundaries(task_or_path, current_state)`
...
2. Mentor derives the current week boundaries.
...
5. Mentor delegates a bounded task to the Junior SWE.
6. Junior SWE implements only within the active week boundaries.
...
- hide future weeks,
- generate current-week learning questions and score the answers,
- update the ledger when the learning check passes,
...
1. the user passes the learning check,
...
- path-level write validation against `active_dirs`,
...
- the user cannot progress without passing the current learning check,
...
4. Implement the learning-question flow.
...
The near-term implementation should not overfit to a specific orchestration stack or pretend that strict isolation already exists. Instead, Phase 1 should deliver the core pedagogical loop with persistent state, hidden future material, artifact tracking, and explicit approval-based progression.
